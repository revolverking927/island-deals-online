...
// Mock "logged-in user" for testing without backend
const mockUser = { role: "manufacturer", email: "test@company.com" };

(async () => {
  // If you want to skip authentication for testing
  const user = mockUser; 
  if (!user) {
    alert("No user found, map will not load");
    return;
  }
...
async function loadParishes(map) {
  // Load local GeoJSON for parishes
  const res = await fetch("/data/geoBoundaries-JAM-ADM1.geojson");
  const data = await res.json();
  ...
  adm1Layer = L.geoJSON(data, {
    ...
    onEachFeature: (feature, layer) => {
      ...
      layer.on({
        mouseover: () => {
          if (currentParish && currentParish !== layer) resetParish(currentParish);
          highlightParish(layer);
        },
        mouseout: () => {
          if (currentParish !== layer) resetParish(layer);
